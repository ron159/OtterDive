...
- ✍️ **Monaco 编辑体验**：按文件名和扩展名识别数十种语言，提供高亮、代码提示、折叠、括号匹配和多光标；超过 20 MiB 的文件进入专用只读分页模式，按需读取、翻页和跳转到原文件行号。
...
大文件模式中的编辑器查找仅覆盖当前页，全文规则分析可使用 Analyse。首次远距离跳转会按需建立行索引；超长行仅预览前 64 KiB。工作区搜索默认跳过超过 20 MiB 的文件，单次结果最多 50,000 条或 32 MiB 文本，取消、截断及跳过原因会在结果中标明。搜索结果历史保留最近五次。
